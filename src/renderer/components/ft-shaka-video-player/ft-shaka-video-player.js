...
export default defineComponent({
  ...
  setup: function (props, { emit, expose }) {
    ...
    const progressBarDuration = ref(0)
    ...
    function hideTopLine() {
      ...
      try {
        ...
        // Znajdź wszystkie span'y
        const spans = textContainer.querySelectorAll('span')

        if (spans.length > 1) {
          // Ukryj wszystkie oprócz ostatniego
          for (let i = 0; i < spans.length - 1; i++) {
            spans[i].style.display = 'none'
          }
        }

        // DODATKOWO: Ukryj div'y z ukrytymi span'ami
    ...
    function applyCaptionStyles() {
      try {
        ...
        // Determine subtitle source context (unused but kept for potential future use)
        ...
        if (textContainer) {
          // Apply bottom offset to the text container (only if not in temporary lift mode)
          if (!isTemporaryLift) {
    ...
    function liftCaptionsAboveControls() {
      try {
        ...
        if (textContainer) {
          // Calculate lifted position (must clear control bar + progress bar)
          const liftedPosition = CONTROL_BAR_HEIGHT + 20 // 20px extra clearance
          if (process.env.NODE_ENV === 'development') {
            // console.log(`   Setting bottom to ${liftedPosition}px`)
          }
          textContainer.style.setProperty('bottom', `${liftedPosition}px`, 'important')
          if (process.env.NODE_ENV === 'development') {
            // console.log('   Applied style:', textContainer.style.bottom)
          }
        } else {
    ...
    function restoreCaptionPosition() {
      try {
        // Debug: Log caption restore
        if (process.env.NODE_ENV === 'development') {
          // console.log('🔽 RESTORING captions')
        }
        isTemporaryLift = false
        const textContainer = findCaptionTextContainer()

        if (textContainer) {
          const restorePosition = captionContainerBottomOffset.value
          if (process.env.NODE_ENV === 'development') {
            // console.log(`   Restoring bottom to ${restorePosition}px`)
          }
          textContainer.style.setProperty('bottom', `${restorePosition}px`, 'important')
          if (process.env.NODE_ENV === 'development') {
            // console.log('   Applied style:', textContainer.style.bottom)
          }
        } else {
    ...
    function setupCaptionHoverListeners() {
      ...
      const handleMouseMove = (event) => {
        ...
        if (isInControlZone && !isMouseInControlArea) {
        ...
        } else if (!isInControlZone && isMouseInControlArea) {
    ...
    function setupCaptionMutationObserver() {
      ...
      const observerConfig = {
        ...
        subtree: true,
        characterData: true,
        attributes: false // Changed to false to avoid infinite loop
      }
    ...
    function checkAndAutoDisableSubtitles() {
      ...
      try {
        ...
        if (systemLangCode === videoLanguage) {
    ...
    function handleTimeupdate() {
      ...
      if (customProgressBarEnabled.value && video.value) {
        ...
        progressBarDuration.value = video.value.duration || 0
