...
import { batchProcessVideoTitles } from '../../helpers/title-language'
...
async function getChannelHomeLocal() {
  ...
  try {
    ...
    // Apply TitleLangFix to video content in home tab shelves
    console.warn('[TitleLangFix][Home] Starting batch process for home tab shelves')
    for (const shelf of homeData_) {
      const videoItems = shelf.content.filter(item => item.type === 'video')
      if (videoItems.length > 0) {
        const processedVideos = await batchProcessVideoTitles(videoItems, { isListContext: true, source: 'channel_home' })
        // Replace the video items in the shelf with processed ones
        let videoIndex = 0
        shelf.content = shelf.content.map(item => {
          if (item.type === 'video') {
            return processedVideos[videoIndex++]
          }
          return item
        })
      }
    }
    console.warn('[TitleLangFix][Home] Batch process completed for home tab')
...
watch(() => currentTab.value, (newTab) => {
  if (newTab === 'videos' && latestVideos.value.length > 0) {
    console.warn('[Channel][Videos] Tab activated, refreshing videos')
    if (process.env.SUPPORTS_LOCAL_API && apiUsed === 'local') {
      isElementListLoading.value = true
      latestVideos.value = []
      videoContinuationData.value = null
  ...
  } else if (newTab === 'home' && homeData.value.length > 0) {
    console.warn('[Channel][Home] Tab activated, refreshing home')
    if (process.env.SUPPORTS_LOCAL_API && apiUsed === 'local') {
      isElementListLoading.value = true
      homeData.value = []
      getChannelHomeLocal()
...
async function getChannelVideosLocal() {
  isElementListLoading.value = true
  const expectedId = id.value
  console.warn('[Channel][Videos] Loading videos tab, count:', latestVideos.value.length)
  ...
  try {
    ...
    console.warn('[TitleLangFix][Channel] Starting batch process for', latestVideos.value.length, 'videos')
    latestVideos.value = await batchProcessVideoTitles(latestVideos.value, { isListContext: true, source: 'channel' })
    console.warn('[TitleLangFix][Channel] Batch process completed for', latestVideos.value.length, 'videos')
...
async function getChannelVideosLocalMore() {
  try {
    ...
    if (isArtistTopicChannel.value) {
      ...
      newVideos = continuation.items.map(parseLocalPlaylistVideo)
      ...
      newVideos = parseLocalChannelVideos(continuation.videos, id.value, channelName.value)
      videoContinuationData.value = continuation.has_continuation ? continuation : null
    }

    // Apply TitleLangFix to newly loaded videos
    console.warn('[TitleLangFix][Channel][Pagination] Starting batch process for', newVideos.length, 'new videos')
    const processedNewVideos = await batchProcessVideoTitles(newVideos, { isListContext: true, source: 'channel_pagination' })
    console.warn('[TitleLangFix][Channel][Pagination] Batch process completed for', processedNewVideos.length, 'videos')

    // Append processed videos to existing list
    latestVideos.value = latestVideos.value.concat(processedNewVideos)
  } catch (err) {
    console.error(err)
    const errorMessage = t('Local API Error (Click to copy)')
    showToast(`${errorMessage}: ${err}`, 10000, () => {
