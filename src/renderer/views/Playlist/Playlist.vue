...
async function getPlaylistLocal() {
  try {
    ...
    let playlistItems_ = result.items.map(parseLocalPlaylistVideo)

    // Apply title language restoration
    playlistItems_ = await batchProcessVideoTitles(playlistItems_, { isListContext: true })
...
async function getNextPageLocal() {
  ...
  if (result) {
    let parsedVideos = result.items.map(parseLocalPlaylistVideo)

    // Apply title language restoration
    parsedVideos = await batchProcessVideoTitles(parsedVideos, { isListContext: true })
