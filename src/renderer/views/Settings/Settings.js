...
import CustomProgressBarSettings from './CustomProgressBarSettings/CustomProgressBarSettings.vue'
...
export default defineComponent({
  ...
  components: {
    ...
    'caption-settings': CaptionSettings,
  ...
  computed: {
    ...
    settingsComponentsData: function () {
      const settingsComponentsData = [
        ...
        {
          type: 'caption-settings',
          title: this.$t('Settings.Caption Settings.Caption Settings'),
          icon: 'closed-captioning'
        },
        {
          type: 'custom-subtitle-settings',
          title: this.$t('Settings.Subtitle Settings.Subtitle Settings'),
          icon: 'font'
        },
        {
          type: 'custom-progress-bar-settings',
          title: this.$t('Settings.Custom Progress Bar Settings.Custom Progress Bar Settings'),
          icon: 'chart-bar'
        },
        ...(process.env.IS_ELECTRON
          ? [{
  ...
  methods: {
    handleMounted: function () {
      ...
      // Only do this if no specific section was requested
      if (this.settingsSectionComponents.length > 0 && !this.$route.query.section) {
        nextTick(() => {
    ...
    navigateToInitialSection: function() {
      // Check if a specific settings section was requested via query parameter
      const requestedSection = this.$route.query.section

      if (requestedSection) {
        // Verify the section exists before navigating to it
        const sectionExists = this.settingsSectionComponents.some(
          section => section.type === requestedSection
        )

        if (sectionExists) {
          // Wait for all components to be mounted before navigating
          nextTick(() => {
            this.navigateToSection(requestedSection)
          })
        }
      }
    },
    ...
    navigateToSection: function(sectionType) {
      if (this.isInDesktopView) {
        // Wait for DOM to be fully ready before attempting to scroll
        nextTick(() => {
          nextTick(() => {
