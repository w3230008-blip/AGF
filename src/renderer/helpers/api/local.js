...
import { batchProcessVideoTitles } from '../title-language'
...
export async function getLocalTrending(location, tab, instance) {
  ...
  // Apply title language restoration
  results = await batchProcessVideoTitles(results, { isListContext: true, source: 'trending' })
...
export async function getLocalVideoInfo(id) {
  ...
  // Store WEB client's full adaptive formats (includes ALL audio tracks metadata)
  const webAdaptiveFormats = info.streaming_data?.adaptive_formats || []
  ...
  webAdaptiveFormats.forEach(format => {
    if (format.has_audio && format.language) {
      availableAudioLanguages.add(format.language)
    }
  })
  ...
  if (mwebInfo.playability_status.status === 'OK' && mwebInfo.streaming_data) {
    ...
    clientName = 'MWEB'
    console.warn('[MultiAudio] Using MWEB client for playback')
  } else {
    console.warn('[MultiAudio] MWEB fallback failed, using WEB client formats')
  }
  ...
  if (info.streaming_data) {
    ...
    if (firstFormat.url || firstFormat.signature_cipher || firstFormat.cipher || firstFormat.decipher) {
...
async function decipherFormats(formats, player) {
  for (const format of formats) {
    try {
      ...
      // Some formats (especially multi-language audio from WEB client) may not have
      // a URL to decipher yet. Skip them and let them be handled later.
      console.warn(`[MultiAudio] Could not decipher format (itag: ${format.itag}, language: ${format.language}):`, error.message)
    }
  }
}
 ...
 * @param {string} languageCode - Language code (e.g., 'en-US', 'de-DE')
 * @returns {Promise<object|null>} Audio format object with URL, or null if not available
 */
export async function getAudioFormatForLanguage(videoId, languageCode) {
  console.warn(`[MultiAudio] Fetching audio for language: ${languageCode}`)

  try {
    const innertube = await createInnertube({ location: 'US', safetyMode: false, clientType: 'WEB', generateSessionData: true })

    // Set the language in the context before fetching
    innertube.session.context.client.hl = languageCode

    // Fetch with MWEB client to get playable URLs
    const info = await innertube.getBasicInfo(videoId, { client: 'MWEB' })

    if (info.playability_status.status !== 'OK' || !info.streaming_data) {
      console.warn(`[MultiAudio] Failed to fetch audio for ${languageCode}: ${info.playability_status.status}`)
      return null
    }
    ...
    if (audioFormats.length === 0) {
      console.warn(`[MultiAudio] No audio formats found for ${languageCode}`)
      return null
    }

    // Decipher the formats
    await decipherFormats(audioFormats, innertube.session.player)

    // Return the best quality audio format
    ...
    console.warn(`[MultiAudio] Found audio for ${languageCode}: itag ${bestAudio.itag}, ${bestAudio.bitrate} bps`)

    return {
      url: bestAudio.freeTubeUrl || bestAudio.url,
    ...
    console.error(`[MultiAudio] Error fetching audio for ${languageCode}:`, error)
    return null
  }
}
...
export async function getLocalChannelVideos(id) {
  ...
  try {
    ...
    // Apply title language restoration
    // console.warn('[TitleLangFix][getLocalChannelVideos] Starting batch process for', videos.length, 'videos')
    videos = await batchProcessVideoTitles(videos, { isListContext: true, source: 'channel' })
    // console.warn('[TitleLangFix][getLocalChannelVideos] Batch process completed for', videos.length, 'videos')
...
export async function getLocalChannelLiveStreams(id) {
  ...
  try {
    ...
    // Apply title language restoration
    // console.warn('[TitleLangFix][getLocalChannelLiveStreams] Starting batch process for', videos.length, 'videos')
    videos = await batchProcessVideoTitles(videos, { isListContext: true, source: 'channel_livestreams' })
    // console.warn('[TitleLangFix][getLocalChannelLiveStreams] Batch process completed for', videos.length, 'videos')
...
async function handleSearchResponse(response) {
  ...
  console.warn('[TitleLangFix] Search integration - starting to process search results')
  ...
  // Apply title language restoration to video items
  const videoResults = results.filter(item => item.type === 'video')
  const channelResults = results.filter(item => item.type === 'channel')
  const playlistResults = results.filter(item => item.type === 'playlist')
  const otherResults = results.filter(item => !['video', 'channel', 'playlist'].includes(item.type))

  console.warn(`[TitleLangFix] Search integration - parsed ${results.length} items: ${videoResults.length} videos, ${channelResults.length} channels, ${playlistResults.length} playlists, ${otherResults.length} others`)

  if (videoResults.length > 0) {
    console.warn(`[TitleLangFix] Search integration - batch processing ${videoResults.length} videos`)
    const processedVideos = await batchProcessVideoTitles(videoResults, { isListContext: true, source: 'search' })

    // Replace video items with processed ones
    let processedIndex = 0
    results = results.map(item => {
      if (item.type === 'video') {
        return processedVideos[processedIndex++]
      }
      return item
    })
  }

  console.warn('[TitleLangFix] Search integration - complete')
