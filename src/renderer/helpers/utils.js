...
// Track if we've already logged missing publishedText for QA debugging
let hasLoggedMissingPublishedText = false
...
export function calculatePublishedDate(publishedText, isLive = false, isUpcoming = false, premiereDate = undefined) {
  ...
  if (!publishedText) {
    // Log once per session for QA debugging
    if (!hasLoggedMissingPublishedText) {
      console.warn('[Dates] publishedText is missing but the video isn\'t live or upcoming')
      hasLoggedMissingPublishedText = true
    }
    return null
  }
