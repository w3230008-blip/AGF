...
const state = {
  ...
  captionBackgroundColor: 'rgba(0, 0, 0, 0.8)',
  captionContainerBottomOffset: 0,
  captionFontSizeFullscreen: 32,
  captionFontColorFullscreen: '#FFFFFF',
  captionFontFamilyFullscreen: 'Arial',
  captionBackgroundColorFullscreen: 'rgba(0, 0, 0, 0.8)',
  captionContainerBottomOffsetFullscreen: 0,
  ...
  customProgressBarMainColor: '#e74c3c',
  customProgressBarHoverColor: '#c0392b',
  customProgressBarBackgroundColor: 'rgba(255, 255, 255, 0.2)',
  customProgressBarBufferedColor: 'rgba(255, 255, 255, 0.4)',
  customProgressBarOpacity: 0.9,
  customProgressBarHeight: 6,
  customProgressBarShowChapterMarkers: true,
  customProgressBarProgressOpacity: 100,
  customProgressBarBufferOpacity: 100,
  customProgressBarUnbufferedOpacity: 100,
  customProgressBarPosition: 0,

  // Custom Subtitle Settings
  customSubtitlesEnabled: false,
  customSubtitlesColor: '#FFFFFF',
  customSubtitlesFontSize: 24,
  customSubtitlesBackgroundColor: '#000000',
  customSubtitlesBackgroundOpacity: 0.8,
  customSubtitlesVerticalPosition: 60,
  customSubtitlesTimeOffset: 0,
  customSubtitlesContainerWidth: 100,
  customSubtitlesColorFullscreen: '#FFFFFF',
  customSubtitlesFontSizeFullscreen: 32,
  customSubtitlesBackgroundColorFullscreen: '#000000',
  customSubtitlesBackgroundOpacityFullscreen: 0.8,
  customSubtitlesVerticalPositionFullscreen: 80,
  customSubtitlesTimeOffsetFullscreen: 0,
  customSubtitlesContainerWidthFullscreen: 100,
...
for (const settingId of Object.keys(state)) {
  ...
  // Add debug logging for customSubtitlesEnabled
  if (settingId === 'customSubtitlesEnabled') {
    getters[getterId] = (state) => {
      const value = state[settingId]
      console.log('[LS DEBUG] STORE getCustomSubtitlesEnabled called:', value)
      return value
    }
    mutations[mutationId] = (state, value) => {
      console.log('[LS DEBUG] MUTATION setCustomSubtitlesEnabled:', value)
      state[settingId] = value
    }
  } else if (settingId === 'captionBackgroundColor' || settingId === 'captionBackgroundColorFullscreen') {
    // Add debug logging for caption background color settings
    getters[getterId] = (state) => state[settingId]
    mutations[mutationId] = (state, value) => {
      console.log(`🎨 [Vuex MUTATION] ${mutationId}:`, value)
      state[settingId] = value
    }
  } else {
    getters[getterId] = (state) => state[settingId]
  ...
  if (settingsWithSideEffects.includes(settingId)) {
    ...
    actions[updaterId] = async ({ commit }, value) => {
      try {
        // Add debug logging for caption settings
