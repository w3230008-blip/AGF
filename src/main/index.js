...
function runApp() {
  ...
  const OEMBED_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36'
  const OEMBED_REFERER = 'https://www.youtube.com/'
  ...
  ipcMain.handle(IpcChannels.TITLE_LANG_OEMBED_FETCH, async (event, videoId) => {
    if (!event.senderFrame || !isFreeTubeUrl(event.senderFrame.url)) {
    ...
    try {
      ...
      try {
        ...
        return { ok: false, status, error: 'invalid-json', message: error?.message ?? '' }
      }

      const title = typeof data?.title === 'string' ? data.title : null

      return { ok: true, status, title }
    } catch (error) {
